...
export default function AvvisiPage() {
  ...
  useEffect(() => {
    ...
    ingredientsApi.priceAlerts(threshold)
      ...
      .catch(() => toast.error(en ? 'Error loading alerts' : 'Errore nel caricamento degli avvisi'))
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              <BellRing size={22} className="text-brand-400" /> {en ? 'Price Alerts' : 'Avvisi Prezzi'}
            </h1>
            <p className="text-dark-200 text-sm mt-1">{en ? 'Ingredients with a rise above the threshold' : 'Ingredienti con un rincaro oltre la soglia'}</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-dark-300">{en ? 'Threshold' : 'Soglia'}</span>
        ...
        {loading ? (
          ...
          <div className="card-dark text-center py-16">
            ...
            <p className="text-white font-medium">{en ? `No rise above +${threshold}%` : `Nessun rincaro oltre +${threshold}%`}</p>
            <p className="text-dark-300 text-sm mt-1">{en ? 'Your ingredient prices are under control.' : 'I prezzi dei tuoi ingredienti sono sotto controllo.'}</p>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm text-orange-400 mb-1">
              ...
              {en
                ? `${alerts.length} ${alerts.length === 1 ? 'ingredient' : 'ingredients'} with a rise above +${threshold}%`
