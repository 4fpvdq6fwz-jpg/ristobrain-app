...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    calcApi.pricingSuggestions(menuId, targetFc).then(res => {
      setData(res.data);
      setLoading(false);
    }).catch(() => { toast.error('Errore nel calcolo'); setLoading(false); });
  }, [menuId, targetFc]);
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Suggerimenti Prezzi</h1>
          <p className="text-dark-200 text-sm mt-1">Prezzi ottimali basati su food cost target</p>
        </div>
        ...
        <div className="card-dark mb-5 flex flex-wrap items-center gap-5">
          ...
          <div className="ml-auto">
            <div className="text-xs text-dark-300">Legenda</div>
            <div className="flex items-center gap-3 mt-1 text-xs">
              <span className="flex items-center gap-1 text-green-400"><TrendingUp size={12} /> Aumenta</span>
              <span className="flex items-center gap-1 text-red-400"><TrendingDown size={12} /> Riduci</span>
        ...
        {loading ? (
          <div className="text-center py-20 text-dark-300">Calcolo suggerimenti...</div>
          ...
          <div className="card-dark overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-dark-600">
                  ...
                  <th className="text-right py-3 px-4 text-dark-200 font-medium">Costo/porz.</th>
                  <th className="text-right py-3 px-4 text-dark-200 font-medium">Prezzo attuale</th>
                  <th className="text-right py-3 px-4 text-dark-200 font-medium">FC% attuale</th>
                  <th className="text-right py-3 px-4 text-dark-200 font-medium">Prezzo suggerito</th>
                  <th className="text-right py-3 px-4 text-dark-200 font-medium">Differenza</th>
                  <th className="text-center py-3 px-4 text-dark-200 font-medium">Azione</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <span className={`text-xs font-medium ${
                          ...
                          {item.action === 'ok' ? 'Ok' : item.action === 'increase' ? 'Aumenta' : 'Riduci'}
                        </span>
                      </div>
                    </td>
                  </tr>
            ...
            <div className="p-4 border-t border-dark-600 text-xs text-dark-300">
              * Prezzi suggeriti calcolati per raggiungere un food cost del <strong className="text-brand-400">{targetFc}%</strong>.
              Formula: Prezzo suggerito = Costo porzione ÷ FC% target.
            </div>
          </div>
        ) : (
          <div className="text-center py-24 text-dark-300">
            <div className="text-5xl mb-4">💰</div>
            <p>{menuId ? 'Nessun piatto con food cost calcolato.' : 'Seleziona un menu per vedere i suggerimenti.'}</p>
          </div>
        )}
      </div>
    </AppLayout>
