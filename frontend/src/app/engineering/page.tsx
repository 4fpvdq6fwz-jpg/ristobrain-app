...
const QUADRANT_INFO: Record<string, { label: string; emoji: string; desc: string; color: string }> = {
  star:      { label: 'Star ⭐', emoji: '⭐', desc: 'Alta popolarità + Alto margine → Promuovi!', color: 'text-yellow-400 bg-yellow-500/15 border-yellow-500/30' },
  puzzle:    { label: 'Puzzle 🧩', emoji: '🧩', desc: 'Bassa popolarità + Alto margine → Migliora visibilità', color: 'text-blue-400 bg-blue-500/15 border-blue-500/30' },
  plowhorse: { label: 'Plowhorse 🐎', emoji: '🐎', desc: 'Alta popolarità + Basso margine → Riduci costo', color: 'text-orange-400 bg-orange-500/15 border-orange-500/30' },
  dog:       { label: 'Dog 🐕', emoji: '🐕', desc: 'Bassa popolarità + Basso margine → Rimuovi?', color: 'text-red-400 bg-red-500/15 border-red-500/30' },
};
...
export default function EngineeringPage() {
  ...
  useEffect(() => {
    ...
    calcApi.engineering(periodId).then(res => {
      setData(res.data);
      setLoading(false);
    }).catch(() => { toast.error('Errore analisi'); setLoading(false); });
  }, [periodId]);
    ...
    <AppLayout>
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">Analisi Menu</h1>
        ...
        {loading ? (
          <div className="text-center py-24 text-dark-300">Calcolo in corso...</div>
        ) : data ? (
          <>
            {/* Summary KPIs */}
              ...
              {Object.entries(QUADRANT_INFO).map(([key, info]) => {
                ...
                return (
                  <div key={key} className={clsx('card-dark border text-center', info.color.split(' ').slice(1).join(' '))}>
                    ...
                    <p className="text-xs text-dark-300">piatti</p>
                  </div>
                );
              })}
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-dark-600">
                      ...
                      <th className="text-right py-3 px-4 text-dark-200 font-medium">Qty venduta</th>
                      ...
                      <th className="text-right py-3 px-4 text-dark-200 font-medium">Margine</th>
                      <th className="text-right py-3 px-4 text-dark-200 font-medium">CM Totale</th>
                      <th className="text-center py-3 px-4 text-dark-200 font-medium">Quadrante</th>
                    </tr>
                  </thead>
                  <tbody>
                  ...
                  {Object.entries(QUADRANT_INFO).map(([key, info]) => {
                    ...
                    return (
                      <div key={key} className={clsx('border rounded-xl p-4', info.color.split(' ').slice(1).join(' '))}>
                        ...
                        <p className="text-xs text-dark-300 mb-3">{info.desc}</p>
                        <div className="space-y-1.5 overflow-y-auto max-h-32">
                          {items.map((item: any, i: number) => (
                            <div key={i} className="flex items-center justify-between text-xs">
                              ...
                              <span className="text-dark-300 ml-2 whitespace-nowrap">{item.qty_sold} pz · €{item.contribution_margin.toFixed(2)}</span>
                            </div>
                          ))}
                          {items.length === 0 && <p className="text-dark-400 text-xs">Nessun piatto in questo quadrante</p>}
                        </div>
                      </div>
                    );
                  })}
          ...
          <div className="text-center py-24 text-dark-300">
            ...
            <p>Seleziona un periodo per iniziare l'analisi.</p>
          </div>
        )}
      </div>
    </AppLayout>
